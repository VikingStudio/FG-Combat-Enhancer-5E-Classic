	...
	* You don't need the class information, but then you can use the path information to see which image record value that the subwindow is pointing to.
	...
	--if w then
		...
		if tokenMap then
			...
			--local optionAutoScale = OptionsManager.getOption('TASG'); -- Settings > Token(GM) > Auto-scale to grid. { labels = "option_val_scale80|option_val_scale100", values = "80|100", baselabel = "option_val_off", baseval = "off", default = "80" });
			local optionOverrideAutoScale = OptionsManager.getOption('CE_OSTG'); -- Settings > 5e Combat Enhancer > Blood splatter, override 'Auto-scale to grid'
			
			if optionOverrideAutoScale == 'off' then
				tokenMap.setScale(((Ss*Is)/Id)*scale); 
				Debug.console('auto-scale blood splatter')				
			end
			if 	optionOverrideAutoScale == 'on' then						
				tokenMap.setScale(Is/Id*scale); 				
				Debug.console('blood splatter auto-scaling overridden')	
			end							
			if optionOverrideAutoScale == 'option_50' then
				tokenMap.setScale(0.5*scale); 
				Debug.console('50% blood splatter scaling')						
			end
